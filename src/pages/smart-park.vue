<template>
  <div class="smart-park">
    <!-- 顶部展示区 -->
    <main-content :mainObj="$t('smartPark.mainObj')"></main-content>
    <!-- 步骤导航 -->
    <step-nav :list="$t('smartPark.stepList')" @clickStep="clickStepItem"></step-nav>
    <!-- 产品特性模块 -->
    <div class="step-one-box" ref="stepItem1">
      <div class="top-title-box">
        <step-title :num="$t('smartPark.stepList')[0].num" :title="$t('smartPark.stepList')[0].title"></step-title>
        <div class="step-btn-box">
          <div
            class="btn-item"
            :class="{'active': activeBtnIndex === index}"
            v-for="(item, index) in $t('smartPark.stepBtnList')" 
            :key="index"
    ...
    <!-- 特性列表组件 -->
    <!-- 商业决策 -->
    <content-introduction v-if="activeBtnIndex === 0" :list="$t('smartPark.listSYJC')"></content-introduction>
    <!-- 数字孪生、智能运维、基础平台 -->
    <transparent-card v-if="activeBtnIndex" :listObj="compList"></transparent-card>
    <!-- 产品优势卡片区域 -->
    <div class="step-two-box" ref="stepItem2">
      <step-title :num="$t('smartPark.stepList')[1].num" :title="$t('smartPark.stepList')[1].title"></step-title>
    </div>
    <top-bottom-card :list="$t('smartPark.cpysList')"></top-bottom-card>
    <!-- 推荐产品卡片区域 -->
    <div class="step-three-box" ref="stepItem3">
      <step-title :num="$t('smartPark.stepList')[2].num" :title="$t('smartPark.stepList')[2].title"></step-title>
      <main-title :title="$t('smartCity.titleObj.title1')"></main-title>
      <!-- 左右布局卡片组件 -->
      <left-right-card :list="$t('smartPark.list')"></left-right-card>
    </div>
  </div>
</template>
  ...
  computed: {
    compList() {
      ...
      if (this.activeBtnIndex === 1) {
        list = this.$t('smartPark.szlsList')
      }
      if (this.activeBtnIndex === 2) {
        list = this.$t('smartPark.znywList')
      }
      if (this.activeBtnIndex === 3) {
        list = this.$t('smartPark.jcptList')
      }
      return list
    }
  },
