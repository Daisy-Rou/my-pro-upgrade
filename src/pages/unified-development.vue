...
// 计算属性
const mainObj = computed(() => tm('unifiedDevelopment.mainObj'));
const stepList = computed(() => tm('unifiedDevelopment.stepList') || []);
const list = computed(() => tm('unifiedDevelopment.list') || []);
const titleObj = computed(() => tm('unifiedDevelopment.titleObj') || {});
const listSolution = computed(() => tm('unifiedDevelopment.listSolution') || []);
const cpysList = computed(() => tm('unifiedDevelopment.cpysList') || []);

// 方法
const clickStepItem = (index) => {
  // 滚动到对应区域
  const targetRef = `stepItem${index + 1}`;
  // 在Vue 3中，我们需要通过getCurrentInstance获取$refs
